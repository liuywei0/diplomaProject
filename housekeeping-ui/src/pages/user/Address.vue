<template>
    <div class="address">
        <div class="btns">
            <el-button type="primary" size="small" @click="toAdd()" icon="el-icon-s-promotion">添加</el-button>
        </div>
...
export default {
    ...
    methods: {
        ...
        submit(ruleForm) {
            this.$refs[ruleForm].validate((valid) => {
                if (valid) {
                    ...
                    console.log(this.form.userId+"sub")
                    axios({
                        method: 'post',
        ...
        loadData() {
            axios({
                method: 'get',
                url: 'http://localhost:8080/address/findByTel',
                params: { tel: users.state.token }
            }).then((response) => {
                if (response.data.code === 200) {
        ...
        loadUser() {
            axios({
                ...
                if (response.data.code === 200) {
                    ...
                    this.form.userId = this.user.id
                    console.log(this.form.userId+"user")
                } else {
                    alert(response.data.message);
                }
            });
